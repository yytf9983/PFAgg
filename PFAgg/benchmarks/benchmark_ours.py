...
import sys
import math
sys.path.append("E:\\code\\fault-tolerant-secure-agg-main")
...
def harary(n: int, k: int):
    ...
    return graph, [str(id) for id in nodes], neighbors

def init_ours_scenario(scenario):

    publicparam, _, _ = TJLS(
        scenario.nclients, scenario.threshold).Setup(TJL_keysize)
...
def benchmark_ours(clients, server, scenario, repititions):
    ...
    setup_register_client_clock = Clock(
        "setup", "register", "client", scenario)
    setup_register_server_clock = Clock(
        "setup", "register", "server", scenario)
    setup_keysetup1_client_clock = Clock(
        "setup", "keysetup1", "client", scenario)
    setup_keysetup2_client_clock = Clock(
        "setup", "keysetup2", "client", scenario)
    setup_keysetup_server_clock = Clock(
        "setup", "keysetup", "server", scenario)
    online_encrypt_client_clock = Clock(
        "online", "encrypt", "client", scenario)
    online_encrypt_server_clock = Clock(
        "online", "encrypt", "server", scenario)
    online_construct_client_clock = Clock(
        "online", "construct", "client", scenario)
    online_construct_server_clock = Clock(
        "online", "construct", "server", scenario)
    ...
    # The clients
    allekshares = {}
    edges, user_ids, neighbors = harary(
        scenario.nclients, int(math.log(scenario.nclients, 2)))
    for i in range(scenario.nclients):
        ...
        setup_keysetup1_bandwith.measure_sent_data(
            (user_ids, eshares), User.size)
    ...
    for i in range(scenario.nclients):
        ...
        online_encrypt_bandwith.measure_sent_data(
            (user, eshares, Y), User.size)
        allebshares[user] = eshares
        allY[user] = Y

    # drop some clients
    nclientsnew = scenario.nclients - \
        ceil(scenario.dropout * scenario.nclients)
    allY = {idx: y for idx, y in allY.items() if idx <= nclientsnew}
    allebshares = {idx: y for idx,
                   y in allebshares.items() if idx <= nclientsnew}
    ...
    for i in range(nclientsnew):
        online_construct_bandwith.measure_rcvd_data(
            allebshares[i+1], User.size)
        online_construct_client_clock.measure_from_here()
        user, bshares, Yzeroshare = clients[i +
                                            1].online_construct(allebshares[i+1])
        online_construct_client_clock.measure_till_here()
        online_construct_bandwith.measure_sent_data(
            (user, bshares, Yzeroshare), User.size)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if [x for x in sys.argv if x == '-p']:
            dont_run = True
            print("Just printing run details")
        else:
            if len(sys.argv) >= 3:
    ...
    benchmark_utils.benchmark(
        benchmark_ours, init_ours_scenario, dont_run, runs)
