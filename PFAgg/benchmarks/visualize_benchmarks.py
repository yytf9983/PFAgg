...
def plot_time_client(ccs, online):
    ...
    structdata = {100000: deepcopy(structdrops), 10000: deepcopy(
        structdrops), 1000: deepcopy(structdrops)}
    ...
    for s in scenarios:
        ...
        ccs_mean[s.dimension][s.dropout][s.nclients] = np.mean(ccs[s]['round0']) + np.mean(
            ccs[s]['round1']) + np.mean(ccs[s]['round2']) + np.mean(ccs[s]['round4'])
        ccs_std[s.dimension][s.dropout][s.nclients] = np.std(ccs[s]['round0']) + np.std(
            ccs[s]['round1']) + np.std(ccs[s]['round2']) + np.std(ccs[s]['round4'])

        online_mean[s.dimension][s.dropout][s.nclients] = np.mean(
            online[s]['encrypt']) + np.mean(online[s]['construct'])
        online_std[s.dimension][s.dropout][s.nclients] = np.std(
            online[s]['encrypt']) + np.std(online[s]['construct'])
    ...
    for ncli in nclients:
        ...
        for drop in reversed(drops):
            j -= 1
            means = np.array([ccs_mean[10000][drop][ncli],
                             online_mean[10000][drop][ncli]])
            error = np.array([ccs_std[10000][drop][ncli],
                             online_std[10000][drop][ncli]])
            ax.bar(x_pos, means, yerr=error, align='center', alpha=1,
                   ecolor='black', color=clrs[j], hatch=htch, capsize=6, width=wdth)
    ...
    lg = fig.legend(handles=handles, loc='upper left',
                    bbox_to_anchor=(.15, .85), borderaxespad=0)

    plt.savefig(plots_dir + 'time_client_clients.png',
                bbox_extra_artists=(lg,), bbox_inches='tight', dpi=400)
    ...
    for dim in dims:
        ...
        for drop in reversed(drops):
            j -= 1
            means = np.array([ccs_mean[dim][drop][ncli],
                             online_mean[dim][drop][ncli]])
            error = np.array(
                [ccs_std[dim][drop][ncli], online_std[dim][drop][ncli]])
            ax.bar(x_pos, means, yerr=error, align='center', alpha=1,
                   ecolor='black', color=clrs[j], hatch=htch, capsize=6, width=wdth)
        i += 1
    ...
    plt.savefig(plots_dir + 'time_client_dims.png',
                bbox_extra_artists=(lg,), bbox_inches='tight', dpi=400)
...
def plot_comm(ccs_sent, ccs_rcvd, online_sent, online_rcvd):
    ...
    for s in scenarios:
        ...
        ccs_sent_mean[s.dimension][s.dropout][s.nclients] = np.mean(ccs_sent[s]['round0']) + np.mean(
            ccs_sent[s]['round1']) + np.mean(ccs_sent[s]['round2']) + np.mean(ccs_sent[s]['round4'])
        ccs_sent_std[s.dimension][s.dropout][s.nclients] = np.std(ccs_sent[s]['round0']) + np.std(
            ccs_sent[s]['round1']) + np.std(ccs_sent[s]['round2']) + np.std(ccs_sent[s]['round4'])
        ccs_rcvd_mean[s.dimension][s.dropout][s.nclients] = np.mean(
            ccs_rcvd[s]['round1']) + np.mean(ccs_rcvd[s]['round2']) + np.mean(ccs_rcvd[s]['round4'])
        ccs_rcvd_std[s.dimension][s.dropout][s.nclients] = np.std(
            ccs_rcvd[s]['round1']) + np.std(ccs_rcvd[s]['round2']) + np.std(ccs_rcvd[s]['round4'])

        online_sent_mean[s.dimension][s.dropout][s.nclients] = np.mean(
            online_sent[s]['encrypt']) + np.mean(online_sent[s]['construct'])
        online_sent_std[s.dimension][s.dropout][s.nclients] = np.std(
            online_sent[s]['encrypt']) + np.std(online_sent[s]['construct'])
        online_rcvd_mean[s.dimension][s.dropout][s.nclients] = np.mean(
            online_rcvd[s]['construct'])
        online_rcvd_std[s.dimension][s.dropout][s.nclients] = np.std(
            online_rcvd[s]['construct'])
    ...
    for ncli in nclients:
        ...
        for drop in reversed(drops):
            j -= 1
            means = np.array([ccs_sent_mean[10000][drop][ncli] + ccs_rcvd_mean[10000][drop][ncli],
                             online_sent_mean[10000][drop][ncli] + online_rcvd_mean[10000][drop][ncli]])
            error = np.array([ccs_sent_std[10000][drop][ncli] + ccs_rcvd_std[10000][drop][ncli],
                             online_sent_std[10000][drop][ncli] + online_rcvd_std[10000][drop][ncli]])
            ax.bar(x_pos, means / 8/1024/1024, yerr=error/8/1024/1024, align='center',
                   alpha=1, ecolor='black', color=clrs[j], hatch=htch, capsize=6, width=wdth)
        i += 1
    ...
    plt.savefig(plots_dir + 'comm_clients.png',
                bbox_extra_artists=(lg,), bbox_inches='tight', dpi=400)
    ...
    for dim in dims:
        ...
        for drop in reversed(drops):
            j -= 1
            means = np.array([ccs_sent_mean[dim][drop][ncli] + ccs_rcvd_mean[dim][drop][ncli],
                             online_sent_mean[dim][drop][ncli] + online_rcvd_mean[dim][drop][ncli]])
            error = np.array([ccs_sent_std[dim][drop][ncli] + ccs_rcvd_std[dim][drop][ncli],
                             online_sent_std[dim][drop][ncli] + online_rcvd_std[dim][drop][ncli]])
            ax.bar(x_pos, means/8/1024/1024, yerr=error/8/1024/1024, align='center',
                   alpha=1, ecolor='black', color=clrs[j], hatch=htch, capsize=6, width=wdth)
        i += 1
    ...
    # plt.tight_layout()
    # plt.savefig(plots_dir + 'time_{}_{}_{}.png'.format(entity, scene, drops))
    plt.savefig(plots_dir + 'comm_dims.png',
                bbox_extra_artists=(lg,), bbox_inches='tight', dpi=400)
...
def plot_time_server(ccs, online):
    ...
    plt.savefig(plots_dir + 'time_server_clients.png',
                bbox_extra_artists=(lg,), bbox_inches='tight', dpi=400)
    ...
    plt.savefig(plots_dir + 'time_server_dims.png',
                bbox_extra_artists=(lg,), bbox_inches='tight', dpi=400)
...
def get_time_ms(setup, online, entity):
    ...
    for s in scenarios:
        ...
        if entity == "client":
            setup_mean[s.dimension][s.dropout][s.nclients] = (
                1000 * np.mean(setup[s]['register']), 1000 * np.mean(setup[s]['keysetup1']) + 1000 * np.mean(setup[s]['keysetup2']))
        else:
            setup_mean[s.dimension][s.dropout][s.nclients] = (
                1000 * np.mean(setup[s]['register']), 1000 * np.mean(setup[s]['keysetup']))

        online_mean[s.dimension][s.dropout][s.nclients] = (
            1000 * np.mean(online[s]['encrypt']), 1000 * np.mean(online[s]['construct']))
...
def get_comm(setup_sent, setup_rcvd, online_sent, online_rcvd):
    ...
    for s in scenarios:
        ...
        setup_sent_mean[s.dimension][s.dropout][s.nclients] = (np.mean(
            setup_sent[s]['register']) / 8/1024, np.mean(setup_sent[s]['keysetup1'])/8/1024)
        setup_rcvd_mean[s.dimension][s.dropout][s.nclients] = (0, np.mean(
            setup_rcvd[s]['keysetup1'])/8/1024 + np.mean(setup_rcvd[s]['keysetup2'])/8/1024)

        online_sent_mean[s.dimension][s.dropout][s.nclients] = (np.mean(
            online_sent[s]['encrypt'])/8/1024, np.mean(online_sent[s]['construct'])/8/1024)
        online_rcvd_mean[s.dimension][s.dropout][s.nclients] = (
            0, np.mean(online_rcvd[s]['construct'])/8/1024)
...
#     for d in [0.0,0.3]:
    for dim in [1000, 10000, 100000]:
        for n in [100, 300, 600]:
            ...
            print("00% failures : Sent = {:.2f} KB & {:.2f} KB & {:.2f} KB & {:.2f} KB".format(
                sS[0], sS[1], oS[0], oS[1]))
            print("00% failures : Rcvd = {:.2f} KB & {:.2f} KB & {:.2f} KB & {:.2f} KB".format(
                sR[0], sR[1], oR[0], oR[1]))
            print("00% failures : Totl = {:.2f} KB & {:.2f} KB & {:.2f} KB & {:.2f} KB".format(
                sS[0]+sR[0], sS[1]+sR[1], oS[0]+oR[0], oS[1]+oR[1]))
            sS = setup_sent_mean[dim][0.1][n]
            oS = online_sent_mean[dim][0.1][n]
            sR = setup_rcvd_mean[dim][0.1][n]
            oR = online_rcvd_mean[dim][0.1][n]
            print("10% failures : Sent = {:.2f} KB & {:.2f} KB & {:.2f} KB & {:.2f} KB".format(
                sS[0], sS[1], oS[0], oS[1]))
            print("10% failures : Rcvd = {:.2f} KB & {:.2f} KB & {:.2f} KB & {:.2f} KB".format(
                sR[0], sR[1], oR[0], oR[1]))
            print("10% failures : Totl = {:.2f} KB & {:.2f} KB & {:.2f} KB & {:.2f} KB".format(
                sS[0]+sR[0], sS[1]+sR[1], oS[0]+oR[0], oS[1]+oR[1]))
            print()
            ...
            sR = setup_rcvd_mean[dim][0.0][n]
            oR = online_rcvd_mean[dim][0.0][n]
            print("30% failures : Sent = {:.2f} KB & {:.2f} KB & {:.2f} KB & {:.2f} KB".format(
                sS[0], sS[1], oS[0], oS[1]))
            print("30% failures : Rcvd = {:.2f} KB & {:.2f} KB & {:.2f} KB & {:.2f} KB".format(
                sR[0], sR[1], oR[0], oR[1]))
            print("30% failures : Totl = {:.2f} KB & {:.2f} KB & {:.2f} KB & {:.2f} KB".format(
                sS[0]+sR[0], sS[1]+sR[1], oS[0]+oR[0], oS[1]+oR[1]))
            print()
...
if __name__ == "__main__":
    ...
    client_setup_time, client_online_time, server_setup_time, server_online_time = parsetime(
        ours_data_dir)
    sent_setup_comm, rcvd_setup_comm, sent_online_comm,  rcvd_online_comm = parsecomm(
        ours_data_dir)

    plot_time_client(ccs_client_time, client_online_time)
    plot_comm(ccs_sent_comm, ccs_rcvd_comm,
              sent_online_comm,  rcvd_online_comm)
    ...
    get_comm(sent_setup_comm, rcvd_setup_comm,
             sent_online_comm,  rcvd_online_comm)
